...
/*   Updated: 2020/12/07 16:29:31 by user42           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "./philo_one.h"

int		take_fork(t_philo *philo, int index)
{
...
int		eat(t_philo *philo, t_args *args, int index)
{
...
int		sleep_tight(t_philo *philo, int index)
{
	if (philo->args->times_philo_must_eat < -1
		|| philo->eaten_meals == philo->args->times_philo_must_eat)
		return (EXIT_FAILURE);
...
int		think(t_philo *philo, int index)
{
	if (philo->args->times_philo_must_eat < -1
		|| philo->eaten_meals == philo->args->times_philo_must_eat
		|| philo->time_of_death < get_time())
